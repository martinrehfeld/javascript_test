 ...
 *  custom additions and fixes by Martin Rehfeld (make it run with HTMLUnit)
 *
 ...
 *--------------------------------------------------------------------------*/
 ...
 DrNicTest.Event.addEvent = function(element, type, handler) {
 	if (element.addEventListener) {
 		...
 		element["on" + type] = DrNicTest.Event.handleEvent;
 	}
 };
 ...
 DrNicTest.Event.fixEvent = function(event) {
 	...
 	event.preventDefault = DrNicTest.Event.fixEvent.preventDefault;
 	event.stopPropagation = DrNicTest.Event.fixEvent.stopPropagation;
 	return event;
 };
